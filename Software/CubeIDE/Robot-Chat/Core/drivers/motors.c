...
void Motor_CommandVelLR(h_Motor_t *m, float v_left, float v_right)
{
    v_left  = clampf(v_left,  -ROBOT_V_MAX, ROBOT_V_MAX);
    v_right = clampf(v_right, -ROBOT_V_MAX, ROBOT_V_MAX);
    // Déterminer les modes cibles
    // Utilisation de BRAKE_MODE si la vitesse demandée est exactement 0 pour un arrêt net
    MotorMode target_mode1 = (v_left > 0.01f)  ? FORWARD_MODE : (v_left < -0.01f) ? REVERSE_MODE : BRAKE_MODE;
    MotorMode target_mode2 = (v_right > 0.01f) ? FORWARD_MODE : (v_right < -0.01f) ? REVERSE_MODE : BRAKE_MODE;
    uint32_t ARR = m->htim_pwm->Init.Period;
    // --- MOTEUR 1 ---
    if (m->mode_mot1 != target_mode1) {
        m->target_speed1 = 0.0f;
        // On autorise le changement de sens beaucoup plus tôt (sous 20% de la vitesse max)
        if (m->current_speed1 < (ARR * 0.2f)) {
            m->mode_mot1 = target_mode1;
            Motor_SetMode(m);
        }
    } else {
    ...
    // --- MOTEUR 2 --- (Identique à M1)
    if (m->mode_mot2 != target_mode2) {
        m->target_speed2 = 0.0f;
        if (m->current_speed2 < (ARR * 0.2f)) {
...
void Motor_UpdateSpeed(h_Motor_t *hMotors)
{
	...
	if (hMotors->current_speed1 < hMotors->target_speed1) {
		//ACCÉLÉRATION : On garde une rampe douce (ex: 100)
		hMotors->current_speed1 += 100;
	}
	else if (hMotors->current_speed1 > hMotors->target_speed1) {
		//FREINAGE : On utilise une rampe violente (ex: 1000)
		hMotors->current_speed1 -= 1000;
	}

	// --- MOTEUR 2 ---
	if (hMotors->current_speed2 < hMotors->target_speed2) {
		hMotors->current_speed2 += 100;
	}
	else if (hMotors->current_speed2 > hMotors->target_speed2) {
		hMotors->current_speed2 -= 1000;
	}

	// On s'assure de ne pas dépasser les bornes
	if (hMotors->current_speed1 < 0) hMotors->current_speed1 = 0;
	if (hMotors->current_speed1 > 3200) hMotors->current_speed1 = 3200;
	// Apply PWM
